...
const JobForm = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [statusRes, serviceRes, inventoryRes, sectorsRes, equipmentRes] =
          await Promise.all([
  ...
  useEffect(() => {
    if (sectorId) {
      setFilteredEquipment(
        equipment.filter((eq) => eq.sectorId === parseInt(sectorId))
      );
    } else {
              ...
              <div className="bg-gray-50/50 rounded-xl p-6 border border-gray-100">
                ...
                <div className="space-y-3">
                  {selectedItems.map((item, index) => (
                    <div
                      key={index}
                      className="flex items-center gap-4 p-3 bg-white rounded-lg border border-gray-200 shadow-sm transition-all hover:shadow-md"
                    >
                      <select
                        value={item.itemId}
                        onChange={(e) =>
                      ...
                      >
                        ...
                        {inventory.map((invItem) => (
                          <option key={invItem.id} value={invItem.id}>
                            {invItem.name} (Stock: {invItem.stock})
                          </option>
                        ))}
                      </select>
                      <input
