...
const StockRequestForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await apiRequest("/stock-requests", {
        method: "POST",
        body: JSON.stringify(payload),
      });

      if (response.ok) {
        showSuccessToast("Pedido agregado exitosamente");
        navigate("/stock-requests");
      } else {
        const error = await response.json();
        showErrorToast(error.error || "No se pudo agregar el pedido");
      }
    } catch (error) {
      console.error("Error adding stock request:", error);
      showErrorToast("Ocurrió un error inesperado al agregar el pedido");
    }
  };
      ...
      <div className="bg-white/80 backdrop-blur-xl shadow-2xl rounded-2xl border border-white/50 overflow-hidden transition-all duration-300 hover:shadow-indigo-500/10">
        <div className="px-8 py-6 border-b border-gray-200/50 bg-white/40">
          ...
          <p className="mt-1 text-sm text-gray-500">
            Agregue un ítem que necesita pedir.
          </p>
        </div>
          ...
          <div className="grid grid-cols-1 gap-y-8 gap-x-6 sm:grid-cols-6">
            ...
            <div className="sm:col-span-3">
              ...
              <input
                ...
                className="block w-full rounded-xl border-gray-400 bg-gray-50/50 focus:bg-white focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 transition-all duration-200 sm:text-sm py-2.5 px-4"
                placeholder="Ej: Hardware, Cables"
              />
            </div>
