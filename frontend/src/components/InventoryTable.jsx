...
const InventoryTable = () => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        if (response.ok) {
          Swal.fire(
            "Eliminado!",
            "El ítem ha sido eliminado del inventario.",
            "success"
          );
          fetchInventory();
        } else {
          ...
          Swal.fire(
            "Error",
            error.error || "No se pudo eliminar el ítem",
            "error"
          );
        }
      } catch (error) {
  ...
  const exportToExcel = () => {
    ...
    for (let R = range.s.r + 1; R <= range.e.r; ++R) {
      ...
      if (
        stockCell &&
        minStockCell &&
        parseInt(stockCell.v) <= parseInt(minStockCell.v)
      ) {
  ...
  const columns = [
    ...
    },
    {
      ...
      sortable: true,
      cell: (row) => (
        <div className="text-sm text-gray-500">{row.minStock}</div>
      ),
    },
    {
      name: "Acciones",
    ...
    >
      ...
      <div className="bg-white/80 backdrop-blur-xl shadow-2xl rounded-2xl border border-white/50 overflow-hidden">
        ...
        <DataTable
          ...
          noDataComponent={
            <div className="p-4 text-gray-500">No hay ítems para mostrar</div>
          }
          highlightOnHover
          pointerOnHover
        />
